...
namespace time
{
	...
	constexpr uint32 SecondsInDay=MinutesInDay*60;
	...
	Duration()=default
		...
		inline Duration operator*(time_t c)const noexcept
		{
			return Duration(time*c);
		}
		inline Duration operator/(time_t c)const noexcept
		{
			return Duration(time/c);
		}
		friend inline Duration operator*(time_t c, Duration d)const noexcept
		{
			return Duration(c*d.time);
		}
		inline Duration& operator+=(Duration d2)const noexcept
		{
			time+=d2.time;
	...
	public:
		...
		{
			tm tmp=tm{second, minute, hour, day, month, (year<1900)?0:year-1900, 1,1, isDaylightSavingTime};
			time=mktime(&tmp);
		}
		...
		{
			return Duration(time%SecondsInDay);
		}
		Time DateOnly()const noexcept
		{
			return Time(time-time%SecondsInDay);
		}
		...
		{
			return Time(time+tm2*SecondsInDay);
		}
		Time AddWeeks(uint16 tm2)const noexcept
		{
			return Time(time+tm2*SecondsInDay*7);
		}
